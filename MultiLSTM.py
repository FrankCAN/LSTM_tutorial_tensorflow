...
def RNN(X, weights, biases):
    ...
    stacked_lstm = tf.nn.rnn_cell.MultiRNNCell([lstm_cell] * n_layer)
    _init_state = stacked_lstm.zero_state(batch_size, tf.float32)

    outputs, states = tf.nn.dynamic_rnn(stacked_lstm, X_in, initial_state=_init_state, time_major=False)
    ...
    #         # X_in_ = tf.reshape(X_in[:, time_step, :], (batch_size, -1))
    #         (cell_out, states) = stacked_lstm(X_in, states)
    #         outputs.append(cell_out)


    # hidden layer for outputs as the final results
    results = tf.matmul(states[n_layer-1][1], weights['out']) + biases['out']
